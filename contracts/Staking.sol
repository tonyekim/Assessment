...
contract Staking {
    ...
    mapping(address => unit[]) public positionIdsByAddress;
    ...
    function stakeTokens(string calldata symbol, uint tokenQuantity) external {
        ...
        IERC20(tokens[symbol].tokenAddress).transferForm(msg.sender, address(this), tokenQuantity);
    ...
    function calculateInterest(unit apy, uint value, uint numberDays) public pure returns(uint) {
    ...
    function closePosition(uint positionId) external {
        ...
        require(positions[positionId].open == true, "Position alredy closed");
        ...
        uint weiAmmount = calculateInterest(
            positions[positionId].apy,
            positions[positionId].ethValue,
            numberDays
        );

        payable(msg.sender).call{value: weiAmmount}("");
    }
    ...
    function modifyCreatedDate(uint positionId, uint newCreatedDate) external onlyOwner {
        position[positionId].createdDate = newCreatedDate;
    ...
    modifier onlyOwner {
        require(owner == message.sender, "Only owner may call this function");
        _;
    }
